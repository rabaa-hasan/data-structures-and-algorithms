...
const getNames = (arr) => {
  // Solution code here...
  let newArr = arr.map(value => value = value.name.split("").reverse().join(""));
  return newArr;
};
...
const count = (target, input) => {
  ...
  return input.reduce((int, set) => {
    const arrayOfFive = set.filter((value) => { 
      return value === target; 
    });
    const numOfFives = arrayOfFive.length; 
    return int += numOfFives; 
  }, 0);
};
